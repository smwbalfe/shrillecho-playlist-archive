...
type ArtistScraper struct {
	RedisStore *redis.Client
	Spotify    sp.SpotifyClient
	NumWorkers int
}

func (s *ArtistScraper) initialScrapeSetup(ctx context.Context, scrapeID int64, rootArtist string) error {
...
func (s *ArtistScraper) ScrapeArtists(scrapeID int64, rootArtist string, maxDepth int) error {
...
func (s *ArtistScraper) worker(ctx context.Context, scrapeID int64, maxDepth int, wg *sync.WaitGroup) {
	defer wg.Done()
	for {
		select {
		case <-ctx.Done():
...
func (s *ArtistScraper) TriggerArtistScrape(ctx context.Context, scrapeID int64, seedArtist string, depth int) ([]data.Artist, error) {
